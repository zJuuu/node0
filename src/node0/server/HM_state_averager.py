...
from node0.server.HM_averager import DecentralizedAverager
...
class TrainingStateAverager(DecentralizedAverager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if stage_name == "tail":
            self.clip_val = 0.8838835
        else:
            self.clip_val = 0.1767767
            ...
            with self.lock_optimizer:
                if optimizer_step:
                    ...
                    with self.lock_averaged_tensors:
                        ...
                        pre_step_grads = torch.nn.utils.clip_grad_norm_(self.main_parameters, self.clip_val)
                        logger.extra(f"Pre step total_grad: {pre_step_grads:.6f}")
                        ...
                        logger.extra(f"Post step total_grad: {post_step_grads:.6f}")
